...
def check_card(where: str, t: dict, kinds: tuple[str, ...]) -> None:
    ...
    logo = t.get("logo")
    if logo and not os.path.exists(os.path.join(TOOLS, logo)):
        err(where, f"logo {logo!r} not found in img/tools/")
    ...
    for heading, relpath, kind in found:
        ...
        for i, e in enumerate(entries, 1):
            ...
            if kind == "entries":
                ...
                check_card(where, e, CARD_KINDS[kind])

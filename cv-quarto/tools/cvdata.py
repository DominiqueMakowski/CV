...
SECTION_RE = re.compile(
    r"^\#\s+(?P<heading>.+?)\s*$"
    r'|cv-(?P<kind>entries|tools|stats|awards|roles|topics|service|engagement)\(\s*yaml\(\s*"(?P<path>[^"]+)"\s*\)',
    re.MULTILINE,
)
...
def sections(path: str | None = None) -> list[tuple[str, str, str]]:
    ...
    "entries", "tools", "awards", "roles", "topics" or "stats". A heading may
    have more than one call under it, and then appears once per call. Reads the
    document rather than a separate manifest, so adding a section in one place
    is enough.
    """
    path = path or os.path.join(ROOT, "cv.qmd")
    text = io.open(path, encoding="utf-8").read()

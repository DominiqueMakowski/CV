# cv-quarto — Quarto + Typst prototype

A parallel rendering path for the CV, replacing R Markdown + `vitae` + LaTeX with
Quarto + Typst. Every section of the Rmd is ported except the publication list
and the additional-training list; `../DominiqueMakowski_CV.Rmd` is untouched and
still produces the CV that goes out until those two land here.
...
1. **Logos are SVG, not PDF.** Native PDF images landed in Typst 0.14. This is
   ...
   cleaner than the `../img/logos/*.pdf` the LaTeX build uses, not a lossy
   conversion of them. `sussex-brighton.svg` is the variant with "Brighton"
   under the wordmark, unused for now.

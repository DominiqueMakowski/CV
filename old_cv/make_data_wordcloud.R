...
# utility function
replace_and_add <- function(data, target, add_to, delete = TRUE) {
  ...
  data <- data %>%
    ...
    dplyr::select(-newfreq)

  if (delete == TRUE) {
    data <- dplyr::filter(data, word != target)
  }
  data
}
...
# Set Path with publications
path <- "C:/Dropbox/PERSO/website/content/publication"  # If from GH action

# Get paths of pdfs
sources <- list.files(path = path, recursive=TRUE, full.names=TRUE, include.dirs=TRUE, pattern = "pdf$")
...
data <- data.frame(word = rownames(as.matrix(text_clean)), freq = rowSums(as.matrix(text_clean))) %>%
  arrange(desc(freq)) %>%
  replace_and_add("er", "emotion", delete = FALSE) %>%
...
# Sys.sleep(5)
ggsave("img/wordcloud.png", p, dpi=600, width=10, height=5)
Sys.sleep(5)

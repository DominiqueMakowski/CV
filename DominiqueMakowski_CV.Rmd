...
aboutme: |
  \RaggedRight Trained as a clinical neuropsychologist and CBT psychotherapist, I am an assistant professor at the University of Sussex, where I head the [Reality Bending Lab](https://realitybendinglab.com). We study aspects of reality bending (e.g., illusions, beliefs of reality, neuroscience of deception, fake news, altered states of consciousness), the role of the body and emotions in cognition (e.g., emotion regulation and interoception), and methodology (statistics, programming, and neurophysiological techniques). I am also strongly committed to open science, developing open-source software (e.g., easystats, NeuroKit) to make advanced methods more accessible.
...
<!-- NOTES -->
<!-- ----- -->
<!-- - The content is meant to be exhaustive, but items can be commented out (e.g., to tailor a CV to an application, or to create short versions etc.) to remove them from the rendered version. -->
...
# Prefix the title line (`what`) of each entry with its logo, looked up in
# img/logos/, and indent the institution line (`with`) by the same amount so
# that the logo spans both header lines of the entry. Entries with no logo (or
# whose file is missing) keep an empty but reserved slot, so that names stay
# aligned within a section.
add_logos <- function(data, files) {
  files <- ifelse(is.na(files) | files == "", "", paste0("img/logos/", files))
  data$what <- paste0("\\cvlogo{", files, "}", data$what)
  data$with <- paste0("\\cvlogoindent{}", data$with)
  data
}
```
